...
public class ExperimentsIntegerExternalViaJAR {
	...
	public ExperimentsIntegerExternalViaJAR(String[][] variables, ArrayList<String> algoritmos, ArrayList<String> jars)
			throws IOException {
		...
		problemList.add(new ExperimentProblem<>(new MyProblemIntegerExternalViaJAR(variables, jars)));
	...
	static List<ExperimentAlgorithm<IntegerSolution, List<IntegerSolution>>> configureAlgorithmList(
		...
		for (int i = 0; i < problemList.size(); i++) {
			...
			for (int j = 0; j != algoritmos.size(); j++) {
				...
				if (algoritmos.get(j).equals("MOCell")) {
				...
				if (algoritmos.get(j).equals("PAES")) {
				...
				if (algoritmos.get(j).equals("RandomSearch")) {
